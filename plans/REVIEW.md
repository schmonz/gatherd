...
`vm/create-base` builds a reusable EOS Sway base image from the real installer ISO. `vm/test` snapshots it, boots under QEMU, runs `site.yml` twice, and asserts `changed=0`. Run it before repaving. Next step: extend to cover `site-async.yml` and `site-vault.yml`.
